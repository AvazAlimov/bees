...
class LeaderRequisitionAjaxController extends Controller
{
    ...
    public function productions(){
        ...
        $productions = Production::where('state','!=',1)->where('state','!=',-1)->whereIn('user_id', $users)
    ...
    public function realizations(){
        ...
        ->where('state','!=',-1)->whereIn('user_id', $users)
    ...
    public function exports(){
    	$users = Auth::user()->acceptedUsers->pluck('id');
        $exports = Export::where('state','!=',1)->where('state','!=',-1)->whereIn('user_id', $users)
            ->with(['user' => function ($query) {
                $query->with(['region' => function ($quer) {
                    $quer->select('id', 'name');
                }])->with(['city' => function ($quer) {
